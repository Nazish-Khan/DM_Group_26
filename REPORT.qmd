---
title: "Group 26 E-commerce Database"
author: "Group_26"
format: pdf 
geometry: 
      - top=30mm
...
![Figure 2- Relationship Sets](project_pics/Relationship sets.jpg)
...
-   Due to geographical considerations and operational efficiency we will not factor discounts and supplier bulk-buying.

-   All transactions carried using Euro

-   ***Assumed that we have procedures in place for regular backups / recovery to protect against data loss and system failures. - with establish backup schedules, retention policies, and disaster recovery plans to ensure the availability and integrity of the e-commerce database.***
...
kable(head(supplier_data, 5)) %>% kable_styling(font_size = 10)
...
kable(head(customer_data, 5)) %>% kable_styling(font_size = 10)
```
...
-   data_upload folder: Houses new data set uploads, facilitating easy access and management of incoming data.
...
In this final part, we will analyse and visualise our data set and perform a fundamental analysis by manipulating data using RSQLite and visualising them using the package "ggplot2"
...
This bar chart displays the top five best-selling products and the bottom five bestselling products. The Reebok Classic Leather is the best-selling product, whereas the Hunger Games Series has the poorest performance.
...
By observing the payment method throughout the year, we notice that a bank transfer displayed an abnormally high number of transactions in January 2023, while the figure for this payment method seems normal in other periods.
